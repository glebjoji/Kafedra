"""
Модуль с реализацией репозитория студентов для PostgreSQL.
Использует Singleton DatabaseConnection.
"""
...
class Student_repl_db(Student_repl_base):
    ...
    def __init__(self):
        # единое соединение через Singleton
    ...
    def get_k_n_short_list(self, k, n):
        ...
        with self.db.get_cursor() as cur:
            cur.execute(
                "SELECT * FROM student ORDER BY student_id LIMIT %s OFFSET %s",
                (k, offset),
            )
            rows = cur.fetchall()
